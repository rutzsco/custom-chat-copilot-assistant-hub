...
public class SearchLogic<T> where T : IKnowledgeSource
{
    ...
    public SearchLogic(AzureOpenAIClient openAIClient, SearchClientFactory factory, List<string> selectFields, VectorSearchSettings settings)
    {
    ...
    public async Task<IEnumerable<KnowledgeSource>> SearchAsync(string query)
    {
        ...
        var searchOptions = new SearchOptions
        {
            ...
            VectorSearch = new()
            {
                Queries = { new VectorizedQuery(queryEmbeddings.ToArray()) { KNearestNeighborsCount = _settings.KNearestNeighborsCount, Fields = { _settings.EmbeddingFieldName }, Exhaustive = _settings.Exhaustive } }
            }
        };
